...
export const configuration = (
  ...
  return {
    ...
    rabbitmq: {
      uri: overrideValues?.rabbitmq?.uri || process.env.RABBITMQ_URI,
      queue: overrideValues?.rabbitmq?.queue || process.env.RABBITMQ_QUEUE,
      noack: overrideValues?.rabbitmq?.noack || process.env.RABBITMQ_NOACK,
      durable:
        overrideValues?.rabbitmq?.durable || process.env.RABBITMQ_DURABLE,
    },
  };
};
